...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=128,
        ...
        if self.use_ffn:
            ...
            self.dropout = nn.Dropout(dropout, inplace=False) # need inplace=False for cuda 11
            self.linear2 = nn.Linear(dim_feedforward, d_model, bias=ffn_use_bias)
            self.norm2 = NORM_DICT[norm_name](d_model) 
            self.norm2 = NORM_DICT[norm_name](d_model)
            self.dropout2 = nn.Dropout(dropout, inplace=False) # need inplace=False for cuda 11

        self.norm1 = NORM_DICT[norm_name](d_model)
        self.dropout1 = nn.Dropout(dropout, inplace=False) # need inplace=False for cuda 11
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=256,
        ...
        self.dropout1 = nn.Dropout(dropout, inplace=False) # need inplace=False for cuda 11
        self.dropout2 = nn.Dropout(dropout, inplace=False) # need inplace=False for cuda 11
        self.dropout3 = nn.Dropout(dropout, inplace=False) # need inplace=False for cuda 11
